...
kvminit(void)
{
  ...
  kernel_pagetable = (pagetable_t) kalloc();  // allocate a physical page for the top-level page directory
  memset(kernel_pagetable, 0, PGSIZE);

  memset(kernel_pagetable, 0, PGSIZE);   // zero it out, so that all the PTEs are zero.
