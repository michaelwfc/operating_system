...
- MIT 6.S081: Operating System Engineering-gdb-tutorial: https://pdos.csail.mit.edu/6.828/2021/lec/gdb_slides.pdf
...
## 5. Step Through the Code: next, step, continue
...
## 6. Inspect Variables: print

The p (or print) command in gdb is used to evaluate and print the value of an expression, which could be a variable, a complex expression, or a memory address as its proper type.
...
The x command in gdb is used to examine memory in whatever format you specify.

Examine memory (x): View memory at a specific address:

```bash
# - x/x <address>: View the memory as  hexadecimal
# - x/i <address>: View the memory as an instruction for assembly
...
## 10. View the machine-level instructions commands
